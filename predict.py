...
if __name__== '__main__':
    ...
    if type(args.batch_size) == str and args.batch_size.startswith('['):
        elements = args.batch_size.strip('[]').replace(" ", "").split(',')
        for e in elements:
            print("Current batch size is: " + e)
            batch_size = int(e)
            pred=modelRecoverCombine.predict_generator(generate_data_generator(val_data_gen,data,batch_size),steps = data.shape[0]/batch_size,verbose=1)
            print("Evaluation time with batch_size " + e + " is: " + str(time.time()-start_time))
            print('Save Output')
            for i in range(pred.shape[0]):
                progress(i+1,data.shape[0],'Saving output...')
                pred[i]=np.clip(pred[i],0.0,1.0)
                cv2.imwrite(args.predictpath+'/'+os.path.splitext(selectNames[i])[0]+ '_batch_size_' + e + '.jpg', (pred[i]*255).astype(np.uint8))
    else:
        batch_size = int(args.batch_size)
        pred=modelRecoverCombine.predict_generator(generate_data_generator(val_data_gen,data,batch_size),steps = data.shape[0]/batch_size,verbose=1)
        print("Evaluation time with batch_size " + str(batch_size) + " is: " + str(time.time()-start_time))
        print('Save Output')
        for i in range(pred.shape[0]):
            progress(i+1,data.shape[0],'Saving output...')
            pred[i]=np.clip(pred[i],0.0,1.0)
